...
Integrated agent support is checked into Weft as task type definitions. A definition owns behavior for a task kind, including input mode, startup status, command construction, screen-derived status, loading rules, terminal cwd tracking, foreground-command tracking, exit footer behavior, screen resize behavior, and restartability during dashboard `U`. The supported `codex` and `claude` agent kinds fill provider-neutral live title/status and resume metadata through provider-specific session handling, prompt detection, interrupt routing, and upgrade resume behavior. Weft discovers Codex session IDs from local session metadata; it assigns a UUID before launching Claude with `--session-id` so the same task can later use `--resume`.
