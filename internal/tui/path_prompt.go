...
func confirmDetail(confirm confirmKind) string {
	if confirm == confirmUpgradeResume {
		return "Closes idle agent terminals, restarts the supervisor, resumes agent tasks with saved session IDs, starts fresh agent tasks without one, and restarts idle shell task(s) with saved history/cwd. Shell jobs, env mutations, shell variables, and unsubmitted input are not preserved."
	}
	if confirm == confirmScheduleUpgrade {
		return "Arms this dashboard to keep checking blockers and run the same safe upgrade automatically once every agent task is idle/resumable and every shell task is idle. Keep this dashboard open. The upgrade will close idle agent terminals, restart the supervisor, resume saved agent sessions, start fresh agent tasks without one, and restart idle shell task(s) with saved history/cwd. Shell jobs, env mutations, shell variables, and unsubmitted input are not preserved."
	}
	if confirm == confirmDeleteTask {
